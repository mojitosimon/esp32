...
//General stuff
const int BaudPcConnection = 9600; //is used for serial with pc and GPS
const int TIMEOUT = 2000; //ms
const int MAX_DIST=100;//m represent 100% of range
...
void loop() { 
  ...
  if (role == role_ping_out) {//send first recieved later
    ...
    radio.write(char_array, max_payload_size);  // This will block until complete
    ...
    if(latitude2 == 1000.00) {
        latitude2 = 48.689373;//dummy values for test 48.689373 , 2.116541
        longitude2 = 2.116541; 
      }
    ...
    if(d>10 || d<0)//fake values
      ...
      {
       float percentage = 1-d*1000./MAX_DIST;//distance normalized
       Serial.println(percentage);  
       if(percentage<0) percentage =0;
       percentage = percentage * NUM_LEDS; //in leds
       Serial.println(percentage);
       for (int i = 0; i < NUM_LEDS; i++) {
        if(i+1>percentage)
          leds[i].setRGB(0, 0, 0);
